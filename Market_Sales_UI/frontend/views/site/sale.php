...
if (!empty($session->get("amount")) && !empty($session->get("supplierMobileNumber")) && !empty($session->get("buyerMobileNumber"))) {
    $model->sellerMsisdn = $session->get("supplierMobileNumber");
    ...
    <div class="col-lg-6">
        <section class="panel form-wizard" id="w1" style="margin-top: 30px;">
            <header class="panel-heading">
                ...
                <h2 class="panel-title">Make a Sale</h2>
            </header>
            <div class="panel-body">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <?= $form->field($model, 'sellerMsisdn', ['enableAjaxValidation' => true])->textInput(['autofocus' => true, 'required' => TRUE, "placeholder" => "Your mobile number i.e 09xxxxxxxx"]) ?>
                        <?= $form->field($model, 'buyerMsisdn', ['enableAjaxValidation' => true])->textInput(['autofocus' => true, 'required' => TRUE, "placeholder" => "Buyer mobile number i.e 09xxxxxxxx"]) ?>
                        <?= $form->field($model, 'amount', ['enableAjaxValidation' => true])->textInput(['autofocus' => true, 'type' => 'number', 'required' => TRUE, "placeholder" => "Sale total amount"]) ?>
                    </div>
                </div>
            </div>
