...
class Core {
    ...
    private function menuSelections() {
        switch ($_SESSION['menu-selection']) {
            ...
            case "BUS_ROUTES1":
                ...
                break;
            case "NO_ROUTES1":
                $this->gw_response = $this->noBusRoutes1();
                break;
            case "TRAVEL_DATE1":
                $this->gw_response = $this->travelDates1();
                break;
            case "BUS_SCHEDULES1":
                $this->gw_response = $this->busSchedules1();
                break;
            case "NO_SCHEDULED_ROUTES1":
                $this->gw_response = $this->noScheduledRoutes1();
                break;
            case "PASSENGER_NRC1":
                $this->gw_response = $this->passengerNrc1();
                break;
            case "PASSENGER_FNAME1":
                $this->gw_response = $this->passengerFirstname1();
                break;
            case "PASSENGER_LNAME1":
                $this->gw_response = $this->passengerLastname1();
                break;
            case "PASSENGER_MSISDN":
                $this->gw_response = $this->passengerMsisdn();
                break;
            case "PASSENGER_CONFIRM1":
                $this->gw_response = $this->passengerConfirm1();
                break;
            case "MAIN":
            ...
            case "5":
                ...
                if (!empty($result['travel_routes']) && sizeof($result['travel_routes']) > 0) {
                    ...
                    if (strlen($routes_menu) > 0) {
                        $_SESSION['menu-selection'] = "BUS_ROUTES1";
    ...
    private function noScheduledRoutes1() {
        switch ($this->body) {
            case "00":
                $_SESSION['menu-selection'] = "TRAVEL_DATE1";
                $this->body = $this->menus['TRAVEL_DATE1'];
                $this->gw_response = $this->get_response_array();
                break;
            case "#":
                $_SESSION['menu-selection'] = "MAIN";
                ...
                break;
            default:
                $_SESSION['menu-selection'] = "NO_SCHEDULED_ROUTES1";
                $this->body = "Invalid selection." . str_replace("{date}", $_SESSION["travel_date"], $this->menus['NO_SCHEDULED_ROUTES1']);
                $this->gw_response = $this->get_response_array();
                break;
        }
        return $this->gw_response;
        ...
        switch ($this->body) {
            ...
            case "2":
                ...
                if (!empty($result) && sizeof($result) > 0) {
                    ...
                    if (strlen($routes_menu) > 0) {
                        $_SESSION['menu-selection'] = "BUS_SCHEDULES1";
                        $this->body = SharedUtils::strReplace("{buses}", $routes_menu, $this->menus['BUS_SCHEDULES1']);
                        $this->body = str_replace("{travel_date}", $_SESSION["travel_date"], $this->body);
                        $this->gw_response = $this->get_response_array();
                    } else {
                        $this->end_of_session = TRUE;
                    ...
                    $_SESSION['menu-selection'] = "NO_SCHEDULED_ROUTES1";
                    $this->body = str_replace("{date}", $_SESSION["travel_date"], $this->menus['NO_SCHEDULED_ROUTES1']);
                    $this->gw_response = $this->get_response_array();
                }
                break;
            case "00":
                ...
                if (!empty($result['travel_routes']) && sizeof($result['travel_routes']) > 0) {
                    ...
                    if (strlen($routes_menu) > 0) {
                        $_SESSION['menu-selection'] = "BUS_ROUTES1";
                        $this->body = SharedUtils::strReplace("{routes}", $routes_menu, $this->menus['BUS_ROUTES1']);
                        $this->gw_response = $this->get_response_array();
                    } else {
                        $this->end_of_session = TRUE;
                        $_SESSION['menu-selection'] = "";
            ...
            default:
                $_SESSION['menu-selection'] = "TRAVEL_DATE1";
                $this->body = "Invalid selection." . $this->menus['TRAVEL_DATE1'];
                $this->gw_response = $this->get_response_array();
                break;
        }
        return $this->gw_response;
    ...
    private function busRoutes1() {
        if ((isset($_SESSION['current_routes']) && isset($_SESSION['current_routes'][$this->body]))) {
            ...
            if (!empty($result['travel_routes']) && sizeof($result['travel_routes']) > 0) {
                ...
                if (strlen($routes_menu) > 0) {
                    $_SESSION['menu-selection'] = "BUS_ROUTES1";
                    $this->body = "Invalid selection." . SharedUtils::strReplace("{routes}", $routes_menu, $this->menus['BUS_ROUTES1']);
                    $this->gw_response = $this->get_response_array();
                } else {
                    $this->end_of_session = TRUE;
                    $_SESSION['menu-selection'] = "";
    ...
    private function noBusRoutes1() {
        switch ($this->body) {
            ...
            default :
                $this->end_of_session = false;
                $_SESSION['menu-selection'] = "NO_ROUTES1";
                $this->body = "Invalid Selection. " . $this->menus['NO_ROUTES1'];
                $this->gw_response = $this->get_response_array();
                break;
        }
        return $this->gw_response;
    ...
    private function busSchedules1() {
        if ((isset($_SESSION['current_routes']) && isset($_SESSION['current_routes'][$this->body]))) {
            //The user selected a valid option.
            $schedule_details = explode(",", $_SESSION['current_routes'][$this->body]);
            ...
            $this->body = $this->menus['PASSENGER_NRC1'];
            $this->gw_response = $this->get_response_array();
        } elseif ($this->body == "00" && $_SESSION['schedules_navigator']['last_first_opt'] == 0) {
            $_SESSION['menu-selection'] = "TRAVEL_DATE1";
            $this->body = $this->menus['TRAVEL_DATE1'];
    ...
    private function passengerNrc1() {
        switch ($this->body) {
            case "#":
                $_SESSION['menu-selection'] = "MAIN";
    ...
    private function passengerFirstname1() {
        switch ($this->body) {
            case "#":
                $_SESSION['menu-selection'] = "MAIN";
                $this->body = SharedUtils::strReplace("{Marketeer}", $_SESSION['names'], $this->menus['MAIN']);
            ...
            default :
                if (!is_numeric($this->body)) {
                    $_SESSION['FIRSTNAME'] = $this->body;
                    $_SESSION['menu-selection'] = "PASSENGER_LNAME1";
                    $this->body = $this->menus['PASSENGER_LNAME1'];
                    $this->gw_response = $this->get_response_array();
                } else {
                    $_SESSION['menu-selection'] = "PASSENGER_FNAME1";
                    $this->body = "First name cannot be a number." . $this->menus['PASSENGER_FNAME1'];
                    $this->gw_response = $this->get_response_array();
                }
                break;
        }
    ...
    private function passengerLastname1() {
        switch ($this->body) {
            case "#":
                $_SESSION['menu-selection'] = "MAIN";
                $this->body = SharedUtils::strReplace("{Marketeer}", $_SESSION['names'], $this->menus['MAIN']);
            ...
            default :
                if (!is_numeric($this->body)) {
                    $_SESSION['LASTNAME'] = $this->body;
                    $_SESSION['menu-selection'] = "PASSENGER_MSISDN";
                    $this->body = $this->menus['PASSENGER_MSISDN'];
                    $this->gw_response = $this->get_response_array();
                } else {
                    $_SESSION['menu-selection'] = "PASSENGER_LNAME1";
                    $this->body = "Last name cannot be a number." . $this->menus['PASSENGER_LNAME1'];
                    $this->gw_response = $this->get_response_array();
                }
                break;
        }
    ...
    private function passengerMsisdn() {
        switch ($this->body) {
            case "#":
                $_SESSION['menu-selection'] = "MAIN";
                $this->body = SharedUtils::strReplace("{Marketeer}", $_SESSION['names'], $this->menus['MAIN']);
            ...
            default :
                if (SharedUtils::validateMsisdn($this->body)) {
                    ...
                    $_SESSION['menu-selection'] = "PASSENGER_MSISDN";
                    $this->body = "Invalid passenger mobile number entered. " . $this->menus['PASSENGER_MSISDN'];
                    $this->gw_response = $this->get_response_array();
                }
                break;
        }
    ...
    private function passengerConfirm1() {
        switch ($this->body) {
            case "#":
                $_SESSION['menu-selection'] = "MAIN";
                $this->body = SharedUtils::strReplace("{Marketeer}", $_SESSION['names'], $this->menus['MAIN']);
