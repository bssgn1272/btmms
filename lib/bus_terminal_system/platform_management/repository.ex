...
#    Repo.get_by(User, [mobile: mobile, operator_role: "MARKETER"])
    query = from u in User, where: u.mobile == ^mobile and (u.role == "MOP" or u.role == "AGNT")
    User.where(query, limit: 1, order_by: [desc: :id]) |> IO.inspect
  end
...
#                "root_route" => BusTerminalSystem.TravelRoutes.find_by([route_code: sub_route["route_code"]]) |> Poison.encode! |> Poison.decode!,
                "root_route" => route |> Poison.encode! |> Poison.decode!,
